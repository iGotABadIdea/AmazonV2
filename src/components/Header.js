  ...
  return (
    <header>
      <div className='flex items-centre bg-amazon_blue p-1 flex-grow py-2'>
        ...
        <div className='hidden sm:flex items-centre h-10 rounded-md flex grow cursor-pointer bg-yellow-400 hover:bg-yellow-500'>
          <input className='p-2 h-full w-6 flex-grow flex-shrink rounded-l-md focus:outline-none' type='text' />
          <SearchIcon className='h-12 p-4' />
        </div>
        <div className='text-white flex items-centre text-xs space-x-6 mx-6 whitespace-nowrap'>
          ...
          <div onClick={() => router.push('/checkout')} className='relative link flex items-centre'>
            <span className='absolute top-0 right-0 md:right-5 w-4 bg-yellow-400 text-center rounded-full text-black font-bold'>{items.length}</span>
